...
## 5.2 GPT: Generative Pre-Training
...
### 5.2.1 Architecture: Decoder-Only Transformer
...
### 5.2.2 Tokenization: Byte Pair Encoding (BPE)
...
This gives GPT a fixed vocabulary of 40,000 subword units (GPT-2 uses 50,257) that handles arbitrarily rare words without an explicit out-of-vocabulary token. BPE operates directly on byte sequences, making it language-agnostic and robust to arbitrary Unicode text. BERT uses a related but distinct algorithm (WordPiece, Section 5.3.4); both produce similar subword vocabularies but differ in their merge criterion.

### 5.2.3 Pre-Training Objective: Autoregressive Language Modeling
...
### 5.2.4 Fine-Tuning: Task Heads and Input Reformulation
...
### 5.3.1 Architecture: Encoder-Only Transformer
...
### 5.3.2 Pre-Training Objective 1: Masked Language Modeling (MLM)
...
The model outputs a distribution over the vocabulary at each masked position, and cross-entropy is computed against the original token. The bidirectional context makes this harder than autoregressive prediction — the model must use both left and right context simultaneously.

### 5.3.3 Pre-Training Objective 2: Next Sentence Prediction (NSP)
...
WordPiece differs from GPT's BPE (Section 5.2.2) primarily in the merge criterion: BPE merges the most *frequent* pair, while WordPiece merges the pair that most increases the language model likelihood. In practice both produce similar subword vocabularies.

### 5.3.5 Fine-Tuning BERT
...
## 5.4 GPT vs. BERT: Two Philosophies
...
### 5.5.2 GPT-3: In-Context Learning at 175B Parameters
...
### 5.5.3 Scaling Laws: The Predictability of Progress
...
where $L$ is cross-entropy loss and the exponents $\alpha$ are empirically measured ($\alpha_N \approx 0.076$, $\alpha_D \approx 0.095$, $\alpha_C \approx 0.050$ respectively). Key findings:

1. **Smooth, predictable improvement**: Performance improves as a smooth power law across many orders of magnitude. There are no phase transitions at arbitrary scales (though see Section 5.6 on emergent abilities).
...
## 5.6 In-Context Learning and Emergent Abilities
...
The word "emergent" is contentious — some researchers argue these are smooth improvements that merely appear discontinuous due to the choice of metric. But the empirical regularity is real: certain capabilities require model scale to manifest, and the threshold matters for deployment decisions.
...
## 5.7 From Pre-Training to Alignment
...
### 5.7.1 RLHF: Reinforcement Learning from Human Feedback
...
The KL divergence penalty prevents the policy from exploiting reward model weaknesses (reward hacking) by deviating too far from the SFT baseline. $\beta$ controls the tradeoff between reward maximization and distribution fidelity.
